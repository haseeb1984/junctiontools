...
if (!is_file($registryPath)) {
    fwrite(STDERR, "Missing tools.json\n");
    exit(1);
}
...
if (!is_array($registry) || !is_array($registry['tools'] ?? null)) {
    fwrite(STDERR, "Tool registry JSON is invalid.\n");
    exit(1);
}
...
foreach ($registry['tools'] as $toolIndex => $tool) {
    ...
    if (is_string($tool['name'] ?? null)) {
        $registryNames[strtolower($tool['name'])] = $tool['id'];
    }
}
...
foreach ($queue as $index => $entry) {
    ...
    if ($entry['existing_tool_match'] !== null) {
        if (!is_string($entry['existing_tool_match']) || !isset($registryById[$entry['existing_tool_match']])) {
            fwrite(STDERR, "existing_tool_match must reference an existing registry tool at queue entry {$index}.\n");
            exit(1);
        }
    ...
    if (isset($registrySlugs[$slugKey])) {
        fwrite(STDERR, "Duplicate-tool prevention failed: recommended_slug '{$entry['recommended_slug']}' already exists in the tool registry.\n");
        exit(1);
    }
    if (isset($seenSlugs[$slugKey])) {
        fwrite(STDERR, "Duplicate recommended_slug at queue entry {$index}.\n");
        exit(1);
    }
...
if (!is_array($enhancements)) {
    fwrite(STDERR, "enhancements must be an array.\n");
    exit(1);
}
$seenEnhancementClusters = [];
foreach ($enhancements as $index => $enhancement) {
    if (!is_array($enhancement)) {
        fwrite(STDERR, "Enhancement entry {$index} must be an object.\n");
        exit(1);
    }
    foreach (['cluster_id', 'core_query', 'decision', 'existing_tool_match', 'target_tool_slug', 'recommended_slug', 'rationale'] as $field) {
        if (!array_key_exists($field, $enhancement)) {
            fwrite(STDERR, "Enhancement entry {$index} missing {$field}.\n");
            exit(1);
        }
    }
    if ($enhancement['decision'] !== 'enhancement') {
        fwrite(STDERR, "Enhancement entry {$index} must have decision=enhancement.\n");
        exit(1);
    }
    $cluster = $enhancement['cluster_id'];
    if (!is_string($cluster) || !preg_match('/^[a-z0-9]+(?:-[a-z0-9]+)*$/', $cluster) || isset($seenEnhancementClusters[$cluster])) {
        fwrite(STDERR, "Invalid or duplicate enhancement cluster_id at entry {$index}.\n");
        exit(1);
    }
    $seenEnhancementClusters[$cluster] = true;
    if (!is_string($enhancement['core_query']) || trim($enhancement['core_query']) === '') {
        fwrite(STDERR, "Empty enhancement core_query at entry {$index}.\n");
        exit(1);
    }
    $match = $enhancement['existing_tool_match'];
    $targetSlug = $enhancement['target_tool_slug'];
    if (!is_string($match) || !isset($registryById[$match])) {
        fwrite(STDERR, "Enhancement entry {$index} must reference an existing registry tool.\n");
        exit(1);
    }
    if (!is_string($targetSlug) || !isset($registrySlugs[strtolower($targetSlug)]) || $registrySlugs[strtolower($targetSlug)] !== $match) {
        fwrite(STDERR, "Enhancement entry {$index} target_tool_slug does not match existing_tool_match.\n");
        exit(1);
    }
    if (!is_string($enhancement['recommended_slug']) || strtolower($enhancement['recommended_slug']) !== strtolower($targetSlug)) {
        fwrite(STDERR, "Enhancement entry {$index} must reuse the target tool slug rather than create a new slug.\n");
        exit(1);
    }
    if (isset($seenClusters[$cluster])) {
        fwrite(STDERR, "Enhancement cluster '{$cluster}' must not also appear in the generation queue.\n");
        exit(1);
    }
}
...
foreach ($registryNames as $name => $id) {
    foreach ($queue as $entry) {
        if (strtolower($entry['core_query']) === $name) {
            fwrite(STDERR, "Duplicate-tool prevention failed: queue query matches existing tool name '{$name}'.\n");
            exit(1);
        }
    }
}
...
echo sprintf(
    "Generation queue validation passed: %d build/research opportunities, %d enhancements, duplicate-tool prevention enforced.\n",
    count($queue),
    count($enhancements)
);
