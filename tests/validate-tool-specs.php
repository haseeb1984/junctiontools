...
foreach($data['specifications'] as $spec){
 foreach(['spec_version','spec_status','generation_eligible','source','tool','purpose','inputs','outputs','content','ux','seo','privacy_security','acceptance_criteria','quality_gates','publication_policy'] as $field){if(!array_key_exists($field,$spec)){fwrite(STDERR,"Missing spec field: {$field}\n");exit(1);}}
 ...
 foreach(['how_to_use','use_cases','tips'] as $contentField){$items=$spec['content'][$contentField]??null;if(!is_array($items)||count($items)<2){fwrite(STDERR,"User-facing content is incomplete: {$contentField}\n");exit(1);}foreach($items as $item){if(!is_string($item)||trim($item)===''){fwrite(STDERR,"Empty user-facing content item: {$contentField}\n");exit(1);}}}
 if(($spec['ux']['layout']??'')!=='standard JunctionTools layout: title and description, How to Use, then the primary tool form/result area'){fwrite(STDERR,"Standard JunctionTools content order is missing.\n");exit(1);}
}
