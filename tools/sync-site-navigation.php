 ...
 * This intentionally never changes config/tools.json or sitemap.xml.
 *
 * Usage:
 *   php tools/sync-site-navigation.php config/tools.json [--apply]
 */
...
foreach ($registry['tools'] as $tool) {
    ...
    if (($tool['seo']['indexable'] ?? true) !== true) continue;
    $name = trim((string) ($tool['name'] ?? ''));
    if ($name === '') continue;
    $tools[$slug] = ['slug' => $slug, 'name' => $name];
}
ksort($tools);

$esc = static fn(string $v): string => htmlspecialchars($v, ENT_QUOTES, 'UTF-8');
$anchor = static function (array $tool): string use ($esc): string {
    return '<a href="' . $esc($tool['slug']) . '">' . $esc($tool['name']) . '</a>';
};
...
foreach ($files as $file) {
    $path = $root . '/' . $file;
    if (!is_file($path)) {
        fwrite(STDERR, "Required site file missing: {$file}\n");
        exit(1);
    }
    $content = (string) file_get_contents($path);
    $original = $content;

    // Keep the visible tool count synchronized wherever it is present.
    $content = preg_replace('/All \d+ Tools/', 'All ' . count($tools) . ' Tools', $content) ?? $content;

    // Replace legacy internal-link targets using the registry's canonical public slug.
    foreach ($tools as $tool) {
        $frontend = strtolower(trim((string) (($registry['tools'][$tool['slug']]['frontend'] ?? ''))));
        if ($frontend !== '') {
            $legacy = preg_quote(preg_replace('/\.html$/i', '', basename($frontend)) ?: '', '/');
            if ($legacy !== '') {
                $content = preg_replace('/href="' . $legacy . '"/i', 'href="' . $esc($tool['slug']) . '"', $content) ?? $content;
            }
        }
    }

    // Append any newly published tool not already linked, without disturbing existing layout.
    $missing = [];
    foreach ($tools as $tool) {
        if (!preg_match('/href=["\']' . preg_quote($tool['slug'], '/') . '(?:["\'])/i', $content)) {
            $missing[] = $tool;
        }
    }
    ...
    if ($content !== $original) {
        $changed[] = $file;
        if ($apply && file_put_contents($path, $content, LOCK_EX) === false) {
